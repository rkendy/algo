...
public class GenomicRangeQuery {
    ...
    public int[] solution(String S, int[] P, int[] Q) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            count[INDEX_A][i] = countA;
            count[INDEX_C][i] = countC;
            count[INDEX_G][i] = countG;
        }

        int[] result = new int[P.length];
        for (int i = 0; i < P.length; i++) {
            int min = 4;
            if (S.charAt(P[i]) == 'A' || count[INDEX_A][Q[i]] > count[INDEX_A][P[i]])
                min = 1;
            else if (S.charAt(P[i]) == 'C' || count[INDEX_C][Q[i]] > count[INDEX_C][P[i]])
                min = 2;
            else if (S.charAt(P[i]) == 'G' || count[INDEX_G][Q[i]] > count[INDEX_G][P[i]])
                min = 3;
            result[i] = min;
        }
    ...
    public static void main(String[] args) {
        System.out.println(Arrays.equals(new int[] { 2, 4, 1 },
