...
public class MaxProductOfThree {
    public int solution(int[] A) {
        Arrays.sort(A);
        int size = A.length;
        int rightMulti = A[size - 2] * A[size - 3];
        if (A[size - 1] < 0)
            return A[size - 1] * rightMulti;

        int leftMulti = A[0] * A[1];
        return A[size - 1] * (leftMulti > rightMulti ? leftMulti : rightMulti);
    }

    public static void main(String[] args) {
        System.out.println(60 == new MaxProductOfThree().solution(new int[] { -3, 1, 2, -2, 5, 6 }));
        System.out.println(-1 == new MaxProductOfThree().solution(new int[] { -10, -10, -1, -1, -1 }));
    }
}
